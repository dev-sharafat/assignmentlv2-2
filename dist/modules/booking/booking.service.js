...
const createNewBookingIntoDb = async (req) => {
    ...
    const vehicleResult = await db_1.pool.query('SELECT * FROM vehicles WHERE id = $1', [payload.vehicle_id]);
    ...
    // Insert booking
    const bookingResult = await db_1.pool.query(`INSERT INTO bookings (customer_id, vehicle_id, rent_start_date, rent_end_date, total_price)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING *`, [payload.customer_id, payload.vehicle_id, payload.rent_start_date, payload.rent_end_date, total_price]);
...
const getAllBookingsFromDb = async (req) => {
    const user = req.user;
...
const updateBookingStatusInDb = async (req) => {
    ...
    const updatedBookingRes = await db_1.pool.query(`UPDATE bookings
             SET status = $1
             WHERE id = $2
